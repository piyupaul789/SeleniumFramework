...
public class Assert {
	...
	public static void assertTrue(boolean condition, String message) {
		if (!condition) { // if false
			failNotEquals(condition, Boolean.TRUE, message); // print fail
		} else { // if true
			if (message != null) { // check if message is not null
				System.out.println("Pass - Assertion : condition " + condition + " message " + message);
			} else { // check if message null
				System.out.println("Pass - Assertion : condition " + condition);
			}
		}
	}
	...
	/**
	 * Differs from {@link #assertEquals(Object, Object, String)} by not taking
	 * arrays into special consideration hence comparing them by reference. Intended
	 * to be called directly to test equality of collections content.
	 */
	private static void assertEqualsImpl(Object actual, Object expected, String message) {
		...
		if (expected.equals(actual) && actual.equals(expected)) {
			System.out.println("Pass - Assertion : expected [" + expected + "] and found [" + actual + "]");
			return;
		}
		failNotEquals(actual, expected, message);
	}
	...
	/**
	 * Asserts that two arrays contain the same elements in the same order. If they
	...
	public static void assertEquals(byte[] actual, byte[] expected, String message) {
		...
		for (int i = 0; i < expected.length; i++) {
			if (expected[i] != actual[i]) {
				fail(String.format(ARRAY_MISMATCH_TEMPLATE, i, Byte.toString(expected[i]), Byte.toString(actual[i]),
						message));
			}
		}
	}
	...
	public static void assertEquals(int[] actual, int[] expected, String message) {
		...
		for (int i = 0; i < expected.length; i++) {
			if (expected[i] != actual[i]) {
				fail(String.format(ARRAY_MISMATCH_TEMPLATE, i, Integer.toString(expected[i]),
						Integer.toString(actual[i]), message));
			}
		}
	}
	...
	public static void assertEquals(boolean[] actual, boolean[] expected, String message) {
		...
		for (int i = 0; i < expected.length; i++) {
			if (expected[i] != actual[i]) {
				fail(String.format(ARRAY_MISMATCH_TEMPLATE, i, Boolean.toString(expected[i]),
						Boolean.toString(actual[i]), message));
			}
		}
	}
	...
	public static void assertEquals(char[] actual, char[] expected, String message) {
		...
		for (int i = 0; i < expected.length; i++) {
			if (expected[i] != actual[i]) {
				fail(String.format(ARRAY_MISMATCH_TEMPLATE, i, Character.toString(expected[i]),
						Character.toString(actual[i]), message));
			}
		}
	}
	...
	public static void assertEquals(long[] actual, long[] expected, String message) {
		...
		for (int i = 0; i < expected.length; i++) {
			if (expected[i] != actual[i]) {
				fail(String.format(ARRAY_MISMATCH_TEMPLATE, i, Long.toString(expected[i]), Long.toString(actual[i]),
						message));
			}
		}
	}
	...
	/**
	 * This methods check referential equality of given arguments as well as
	 * references length (assuming they are arrays). Successful execution of this
	 * method guaranties arrays length equality.
	 *
	 * @param actualArray
	 *            array of elements
	 * @param expectedArray
	 ...
	 * @param message
	 *            the assertion error message
	 * @return {@code true} if {@code actualArray} and {@code expectedArray} are the
	 *         same, {@code false} otherwise. If references are different and arrays
	 ...
	 * AssertionError, with the given message, is thrown. If the expected value is
	 ...
	 *            the expected value
	 * @param delta
	 *            the absolute tolerable difference between the actual and expected
	 *            values
	 * @param message
	 *            the assertion error message
	 */
	...
	/**
	 * Asserts that two doubles are equal concerning a delta. If they are not, an
	 * AssertionError is thrown. If the expected value is infinity then the delta
	 * value is ignored.
	 * 
	 * @param actual
	 *            the actual value
	...
	/**
	 * Asserts that two longs are equal. If they are not, an AssertionError, with
	 * the given message, is thrown.
	 * 
	 * @param actual
	 *            the actual value
	...
	/**
	 * Asserts that two booleans are equal. If they are not, an AssertionError, with
	 * the given message, is thrown.
	 * 
	 * @param actual
	 *            the actual value
	...
	/**
	 * Asserts that two bytes are equal. If they are not, an AssertionError, with
	 * the given message, is thrown.
	 * 
	 * @param actual
	 *            the actual value
	...
	/**
	 * Asserts that an object isn't null. If it is, an AssertionFailedError, with
	 * the given message, is thrown.
	 * 
	 * @param object
	 *            the assertion object
	...
	/**
	 * Asserts that an object is null. If it is not, an AssertionFailedError, with
	 * the given message, is thrown.
	 * 
	 * @param object
	 *            the assertion object
	...
	/**
	 * Asserts that two collections contain the same elements in the same order. If
	 ...
	 * note that this assert iterates over the elements and modifies the state of
	 * the iterators.
	 * 
	 * @param actual
	 *            the actual value
	...
	/**
	 * Asserts that two arrays contain the same elements in no particular order. If
	 * they do not, an AssertionError, with the given message, is thrown.
	 * 
	 * @param actual
	 *            the actual value
	...
	public static void assertEqualsNoOrder(Object[] actual, Object[] expected, String message) {
		...
		if ((actual == null && expected != null) || (actual != null && expected == null)) {
			failAssertNoEqual("Arrays not equal: " + Arrays.toString(expected) + " and " + Arrays.toString(actual),
					message);
		}
	...
	public static void assertEquals(Map<?, ?> actual, Map<?, ?> expected, String message) {
		...
		for (Object anEntrySet : entrySet) {
			...
			String assertMessage = message != null ? message
	...
	/**
	 * This interface facilitates the use of {@link #expectThrows} from Java 8. It
	 * allows method references to both void and non-void methods to be passed
	 * directly into expectThrows without wrapping, even if they declare checked
	 * exceptions.
	 * <p/>
	...
	/**
	 * Asserts that {@code runnable} throws an exception when invoked. If it does
	 * not, an {@link AssertionError} is thrown.
	 *
	 * @param runnable
	 ...
	 * {@code throwableClass} when executed. If it does not throw an exception, an
	 * {@link AssertionError} is thrown. If it throws the wrong type of exception,
	 * an {@code AssertionError} is thrown describing the mismatch; the exception
	 ...
	 * {@code runnable} does not throw an exception, an {@link AssertionError} is
	 * thrown. If it throws the wrong type of exception, an {@code
	 * AssertionError} is thrown describing the mismatch; the exception that was
	 * actually thrown can be obtained by calling {@link AssertionError#getCause}.
	 *
	 * @param throwableClass
	 *            the expected type of the exception
	 * @param runnable
